...
attribute::verification_type_info read_verification_type_info(data_reader& reader) {
  switch (reader.read_u8()) {
    case attribute::top_variable_info::tag: return attribute::top_variable_info{};
    case attribute::integer_variable_info::tag: return attribute::integer_variable_info{};
    case attribute::float_variable_info::tag: return attribute::float_variable_info{};
    case attribute::double_variable_info::tag: return attribute::double_variable_info{};
    case attribute::long_variable_info::tag: return attribute::long_variable_info{};
    case attribute::null_variable_info::tag: return attribute::null_variable_info{};
    case attribute::uninitialized_this_variable_info::tag: return attribute::uninitialized_this_variable_info{};
    case attribute::object_variable_info::tag: return attribute::object_variable_info{reader.read_u16()};
    case attribute::uninitialized_variable_info::tag: return attribute::uninitialized_variable_info{reader.read_u16()};
    default: throw std::runtime_error("Unknown verification type info tag");
  }
}

attribute::stack_map_frame read_stack_map_frame(data_reader& reader) {
  if (const auto frame_type = reader.read_u8(); frame_type <= 63) {
    return attribute::same_frame{frame_type};
  } else if (frame_type <= 127) {
    return attribute::same_locals_1_stack_item_frame{frame_type, read_verification_type_info(reader)};
  } else if (frame_type == 247) {
    return attribute::same_locals_1_stack_item_frame_extended{reader.read_u16(), read_verification_type_info(reader)};
    ...
    for (auto i = 0; i < number_of_locals; i++) {
      locals.push_back(read_verification_type_info(reader));
    }
    ...
    for (auto i = 0; i < number_of_stack_items; i++) {
      stack.push_back(read_verification_type_info(reader));
    }
...
attribute::annotation read_annotation(data_reader& reader);

attribute::element_value read_element_value(data_reader& reader) {
  switch (const auto tag = reader.read_u8(); tag) {
    ...
    case 'J':
    case 's':
    case 'c': return {tag, reader.read_u16()};
    case 'e': return {tag, attribute::element_value::enum_value{reader.read_u16(), reader.read_u16()}};
    case '@': return {tag, read_annotation(reader)};
    case '[': {
      const auto num_values = reader.read_u16();
      std::vector<attribute::element_value> values;
      values.reserve(num_values);
      for (auto i = 0; i < num_values; i++) {
        values.push_back(read_element_value(reader));
      }
      return {tag, std::move(values)};
    }
    default: throw std::runtime_error("Unknown element value tag");
  }
}
...
attribute::annotation read_annotation(data_reader& reader) {
  ...
  for (auto i = 0; i < num_element_value_pairs; i++) {
    element_value_pairs.push_back({
      reader.read_u16(), read_element_value(reader)
    });
  }
  return {type_index, std::move(element_value_pairs)};
}

attribute::type_annotation read_type_annotation(data_reader& reader) {
  ...
  switch (target_type) {
    ...
    case 0x4B:
      ...
      break;
    default: throw std::runtime_error("Unknown type annotation target type");
  }
...
inline cp::constant_pool read_constant_pool(data_reader& reader) {
  ...
  for (auto i = 1; i < count; i++) {
    switch (const auto tag = reader.read_u8(); tag) {
      ...
      case cp::package_info::tag:
        ...
        break;
      default: throw std::runtime_error("Unknown constant pool tag (" + std::to_string(tag) + ")");
    }
  }
  return pool;
}

attribute::attribute read_attribute(data_reader& reader, const cp::constant_pool& pool, bool oak) {
  ...
  try {
    if (const auto name = std::get_if<cp::utf8_info>(&pool[name_index])) {
      ...
      data_reader r(iss);
      switch (attr_type) {
        case at::constant_value: return {attribute::constant_value{r.read_u16()}};
        case at::code: {
          ...
          for (auto i = 0; i < attributes_count; i++) {
            attributes.push_back(read_attribute(r, pool, oak));
          }
          return {
            attribute::code{max_stack, max_locals, std::move(bytecode), std::move(exceptions), std::move(attributes)}
          };
        }
        case at::stack_map_table: {
          ...
          for (auto i = 0; i < number_of_entries; i++) {
            entries.push_back(read_stack_map_frame(r));
          }
        ...
        case at::enclosing_method: return {attribute::enclosing_method{r.read_u16(), r.read_u16()}};
        case at::synthetic: return {attribute::synthetic{}};
        case at::signature: return {attribute::signature{r.read_u16()}};
        case at::source_file: return {attribute::source_file{r.read_u16()}};
        case at::source_debug_extension: return {attribute::source_debug_extension{std::move(info)}};
        ...
        case at::deprecated: return {attribute::deprecated{}};
        case at::runtime_visible_annotations: {
          ...
          for (auto i = 0; i < num_annotations; i++) {
            annotations.emplace_back(read_annotation(r));
          }
        ...
        case at::runtime_visible_type_annotations: {
          ...
          annotations.reserve(num_annotations);
          for (auto i = 0; i < num_annotations; i++) {
            annotations.emplace_back(read_type_annotation(r));
          }
        ...
        case at::annotation_default: return {attribute::annotation_default{read_element_value(r)}};
        ...
        case at::module_main_class: return {attribute::module_main_class{r.read_u16()}};
        case at::nest_host: return {attribute::nest_host{r.read_u16()}};
...
std::istream& operator>>(std::istream& stream, class_file& file) {
  ...
  const auto oak = major_version < class_version::v1_1 || (major_version == class_version::v1_1 && minor_version < 3);
  auto constant_pool = read_constant_pool(reader);
